...
def SauvegarderDonneesUtilisateur():
    try:
        with open("Authentification.txt", "w") as file:
            ...
            classe = input("Classe (GLSI-3-A/B/C/D) : ")
            email = input("Email : ")
            

            if re.match(r"[^@]+@[^@]+\.[^@]+", email) and re.match(r"GLSI-3-[A-D]", classe):
...
def MenuB1():
    ...
    while True:
        ...
        print("B1-d Revenir au menu MenuB")
...
def DechiffrementAffine():
    ...
    # Vérifier si Ka est premier avec 26
    if math.gcd(Ka, 26) == 1:
        for lettre in message:
            if lettre in alphabet:
                position = alphabet.index(lettre)
                # Calculer la nouvelle position en utilisant l'inverse modulaire de Ka
        ...
        print("La clé Ka doit être premier avec 26 pour assurer l'inversibilité modulaire.")
def MenuB2c():
    while True:
...
def ChiffrementRSA():
    ...
    # Enregistrer la clé privée au format PEM
    ...
    # Enregistrer la clé publique au format PEM
    ...
    # Enregistrer le message chiffré
...
def DechiffrementRSA():
    try:
        ...
        # Charger le message chiffré depuis le fichier
